...
def main(_):
    ...
    is_train_rnn = True
    net_rnn = rnn_embed(t_real_caption, is_train=is_train_rnn, reuse=False, name='g')    # if pre-trained
    net_rnn2 = rnn_embed(t_real_caption, is_train=is_train_rnn, reuse=False, name='d')
    ...
    _, disc_wrong_caption_logits = discriminator_txt2img(
                    t_real_image,
                    rnn_embed(t_wrong_caption, is_train=is_train_rnn, reuse=True, name='d'),                                          # remove if DCGAN only
                    is_train=True, reuse=True)

    # testing inference for txt2img
    net_g, _ = generator_txt2img(t_z,
                    rnn_embed(t_real_caption, is_train=False, reuse=True, name='g'), # remove if DCGAN only
    ...
    eg_vars = tl.layers.get_variables_with_name('rnng', True, True)
    ed_vars = tl.layers.get_variables_with_name('rnnd', True, True)
    d_vars = tl.layers.get_variables_with_name('discriminator', True, True)
    g_vars = tl.layers.get_variables_with_name('generator', True, True)

    # d_optim = tf.train.AdamOptimizer(lr, beta1=beta1).minimize(d_loss, var_list=d_vars + ed_vars)
    # g_optim = tf.train.AdamOptimizer(lr, beta1=beta1).minimize(g_loss, var_list=g_vars + eg_vars)

    ## try different learning rate (better implementation)  ref: https://codedump.io/share/uW7fYwktTOt6/1/how-to-set-layer-wise-learning-rate-in-tensorflow
    opt1 = tf.train.AdamOptimizer(lr, beta1=beta1)
    opt2 = tf.train.AdamOptimizer(lr/5, beta1=beta1)
    grads = tf.gradients(d_loss, d_vars + ed_vars)
    # grads, _ = tf.clip_by_global_norm(grads, 10)      # Truncated grads global
    grads1 = grads[0:len(d_vars)]
    grads2 = grads[-len(ed_vars):]
    # grads2, = = tf.clip_by_global_norm(grads2, 10)      # Truncated
    train_op1 = opt1.apply_gradients(zip(grads1, d_vars))
    train_op2 = opt2.apply_gradients(zip(grads2, ed_vars))
    d_optim = tf.group(train_op1, train_op2)

    opt1 = tf.train.AdamOptimizer(lr, beta1=beta1)
    opt2 = tf.train.AdamOptimizer(lr/5, beta1=beta1)
    grads = tf.gradients(g_loss, g_vars + eg_vars)
    # grads, _ = tf.clip_by_global_norm(grads, 10)      # Truncated grads global
    grads1 = grads[0:len(g_vars)]
    grads2 = grads[-len(eg_vars):]
    # grads2, _ = = tf.clip_by_global_norm(grads2, 10)      # Truncated
    train_op1 = opt1.apply_gradients(zip(grads1, g_vars))
    train_op2 = opt2.apply_gradients(zip(grads2, eg_vars))
    g_optim = tf.group(train_op1, train_op2)
    ...
    with tf.Session() as sess:
        ...
        net_eg_name = os.path.join(save_dir, 'net_eg.npz')
        net_ed_name = os.path.join(save_dir, 'net_ed.npz')
        net_g_name = os.path.join(save_dir, 'net_g.npz')
        net_d_name = os.path.join(save_dir, 'net_d.npz')
        if not (os.path.exists(net_eg_name) and os.path.exists(net_ed_name)):
            print("[!] Loading RNN checkpoints failed!")
        else:
            net_eg_loaded_params = tl.files.load_npz(name=net_eg_name)
            net_ed_loaded_params = tl.files.load_npz(name=net_ed_name)
            tl.files.assign_params(sess, net_eg_loaded_params, net_rnn)
            tl.files.assign_params(sess, net_ed_loaded_params, net_rnn2)
        ...
        for step in range(0, n_step):
            ...
            if (step != 0) and (step % n_check_step) == 0:
                ...
                ## Save model to npz
                tl.files.save_npz(net_rnn.all_params, name=net_eg_name, sess=sess)
                tl.files.save_npz(net_rnn2.all_params, name=net_ed_name, sess=sess)
                tl.files.save_npz(net_g.all_params, name=net_g_name, sess=sess)
                tl.files.save_npz(net_d.all_params, name=net_d_name, sess=sess)
                net_eg_name_ = os.path.join(save_dir, 'net_eg_%d.npz' % step)
                net_ed_name_ = os.path.join(save_dir, 'net_ed_%d.npz' % step)
                net_g_name_ = os.path.join(save_dir, 'net_g_%d.npz' % step)
                net_d_name_ = os.path.join(save_dir, 'net_d_%d.npz' % step)
                tl.files.save_npz(net_rnn.all_params, name=net_eg_name_, sess=sess)
                tl.files.save_npz(net_rnn2.all_params, name=net_ed_name_, sess=sess)
