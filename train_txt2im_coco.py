...
Data augmentation:
1. random crop
    torch  : 76x76 - crop -> 64x64
    us     : 346x346 - crop -> 299x299 - resize -> 64x64
...
3. rotate
    ...
    us :
        x = rotation(x, rg=5, is_random=True, fill_mode='nearest')
"""
...
## dir for MSCOCO, im2txt model and vocabulary
...
## dir for txt2im
save_dir = "checkpoint" # <-- store/restore the GAN model
if not os.path.exists(save_dir):
    print("[!] Folder %s is not exist, create it." % save_dir)
    os.mkdir(save_dir)
sample_dir = "samples"  # <-- save generated images
...
rnn_hidden_size = 512       # lstm hidden size
...
def cnn_embed(input_imgs, is_train, reuse):
...
def rnn_embed(input_seqs, is_train, reuse):#, return_embed=False):
...
def main(_):
    ...
    images, input_seqs, target_seqs, input_mask = Build_Inputs(mode='train', input_file_pattern=input_file_pattern) # by default, it has random brightness, saturation, contrast, hue, flip, crop; but remember to to use flip and crop only.
    ...
    net_cnn = cnn_embed(t_rnn_image, is_train=False, reuse=False)
    x = net_cnn.outputs
    v = rnn_embed(t_rnn_caption, is_train=True, reuse=False).outputs
    x_w = cnn_embed(t_rnn_image_w, is_train=False, reuse=True).outputs
    ...
    t_real_image = tf.placeholder('float32', [batch_size, image_size, image_size, 3], name = 'real_image')      # 313 https://github.com/reedscot/icml2016/blob/master/main_cls.lua
    t_wrong_image = tf.placeholder('float32', [batch_size, image_size, image_size, 3], name = 'wrong_image')    # 314
    t_real_caption = tf.placeholder(dtype=tf.int64, shape=[batch_size, None], name='real_caption_input')        # 315
    ...
    #                 t_real_image,
    #                 rnn_embed(t_wrong_caption, is_train=False, reuse=True),
    #                 is_train=True, reuse=True)
    ...
    net_g, _ = generator_txt2img_resnet(t_z,
                    rnn_embed(t_real_caption, is_train=False, reuse=True),
    ...
    cls_weight = 0.5
    d_loss = d_loss1 + cls_weight * d_loss2 + (1-cls_weight) * d_loss3      # CLS=0.5 # https://github.com/reedscot/icml2016/blob/master/scripts/train_coco.sh
    g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(disc_fake_image_logits, tf.ones_like(disc_fake_image_logits)))  # <- fake image, right(arbitrary) text
    ...
    # d1_optim = tf.train.AdamOptimizer(lr_v, beta1=beta1).minimize(d_loss1, var_list=d_vars ) # train with real  326
    # d2_optim = tf.train.AdamOptimizer(lr_v, beta1=beta1).minimize(d_loss2, var_list=d_vars ) # train with wrong 334
    # d3_optim = tf.train.AdamOptimizer(lr_v, beta1=beta1).minimize(d_loss3, var_list=d_vars ) # train with fake  345



    # opt1 = tf.train.AdamOptimizer(lr, beta1=beta1)
    # opt2 = tf.train.AdamOptimizer(lr, beta1=beta1)    # <- for RNN
    ...
    InceptionV3_var = tl.layers.get_variables_with_name('InceptionV3', False, False)
    saver = tf.train.Saver(InceptionV3_var)
    ...
    for i, sentence in enumerate(sample_sentence):
        ...
        # print("%d : %s" % (i, [vocab.id_to_word(id) for id in sample_sentence[i]]) )
        # print(sample_sentence[i])
    sample_sentence = tl.prepro.pad_sequences(sample_sentence, padding='post')
    ## you may want to check the seed sentence / ['a','man',...,'<\S>','a','a']
    # for i, sentence_id in enumerate(sample_sentence):
    #     print("seed: %d : %s" % (i, [vocab.id_to_word(id) for id in sentence_id]) )
    ...
    with tf.Session() as sess:
        ...
        for step in range(11501, n_step):
            ...
            # b_images = threading_data(b_image_file_name, prepro_img, mode='load_image')
            ...
            ## get img by TFRecord
            # read data  : 66 https://github.com/reedscot/icml2016/blob/master/main_cls.lua
            # data augmentation: https://github.com/reedscot/icml2016/blob/master/data/donkey_folder_coco.lua
            # The images already been random crop from 346 to 299, random flip letf/right and rescale to[-1, 1]
            b_images, b_sentences_ids = sess.run([images, target_seqs])
            b_sentences_ids = list(b_sentences_ids)

            # print(type(b_sentences_ids), len(b_sentences_ids)) # <type 'numpy.ndarray'> 64
            b_images_w, b_sentences_ids_w = sess.run([images, target_seqs]) # <- already random crop to 299, [-1, 1]
            b_sentences_ids_w = list(b_sentences_ids_w) # [a man is cool </S> a a a]

            ## update RNN for image-text mapping
            if False:
                errE, _ = sess.run([e_loss, e_optim], feed_dict={
                                                t_rnn_image : b_images,
                                                t_rnn_image_w : b_images_w,
            ...
            # print(b_images.shape, np.min(b_images), np.max(b_images)) # (64, 299, 299, 3) -1.0 1.0
            # b_images = threading_data(b_images, prepro_img, mode='random_size_to_346')
            # print(b_images.shape, np.min(b_images), np.max(b_images)) # (64, 346, 346, 3) 0 255
            # exit()
            ...
            # for i, sentence_id in enumerate(b_sentences_ids):
            #     print("%d : %s" % (i, [vocab.id_to_word(id) for id in sentence_id]) )
            # print(b_sentences_ids[-1:])
            # print("xx : %s" % ([vocab.id_to_word(id) for id in b_sentences_ids[-1]]) )
            # exit()
            #
            ...
            if step >= 0:
                ...
                ## resize the images from 299x299 to 64x64 and random rotate
                b_images_txt2im = threading_data(b_images, prepro_img, mode='resize_to_64')
                b_images_txt2im_w = threading_data(b_images_w, prepro_img, mode='resize_to_64')
                # print(b_images_txt2im.shape, np.min(b_images_txt2im), np.max(b_images_txt2im)) #(64, 64, 64, 3) -1.0 1.0
                # exit()
                ...
                # print(type(b_wrong_sentences_ids), len(b_wrong_sentences_ids))
                # for i, sentence_id in enumerate(b_wrong_sentences_ids):
                #     print("%d : %s" % (i, [vocab.id_to_word(id) for id in sentence_id]) )
                # exit()
                ...
                # 319 update seperatly ?  https://github.com/reedscot/icml2016/blob/master/main_cls.lua
                # but in the paper, it update once !
                # train with real   326
                # d1,  _ = sess.run([d_loss1, d1_optim], feed_dict={
                #                 t_real_image : b_images_txt2im,
                #                 t_real_caption : b_sentences_ids})
                # # train with wrong  335
                # d2, _ = sess.run([d_loss2, d2_optim], feed_dict={
                #                 t_wrong_image : b_images_txt2im_w,
                #                 t_real_caption : b_sentences_ids})
                # # train with fake   345
                # d3, _ = sess.run([d_loss3, d3_optim], feed_dict={
                #                 t_real_caption : b_sentences_ids,
                #                 t_z : b_z})
                # errD = d1 + cls_weight * d2 + (1-cls_weight) * d3

                for _ in range(1): #<- torch code update G once : line 418 https://github.com/reedscot/icml2016/blob/master/main_cls.lua
            ...
            # print("step %d: d_loss: %.4f (%.3f, %.3f, %.3f), g_loss: %.4f, e_loss: %.4f (%.2f sec)" % (step, errD, d1, d2/2, d3/2, errG, errE, time.time()-start_time))
            print("step %d: d_loss: %.4f (%.3f, %.3f, %.3f), g_loss: %.4f, e_loss: %.4f (%.2f sec)" % (step, errD, d1, d2, d3, errG, errE, time.time()-start_time))
            if step != 0 and step % n_check_step == 0:
                ## Print average loss
                # print(" ** d_loss: %.8f, g_loss: %.8f: " % (total_d_loss/n_check_step, total_g_loss/n_check_step))
                log = " ** avg step: %d d_loss: %.4f (%.3f, %.3f, %.3f), g_loss: %.4f, e_loss: %.4f: " % (step, total_d_loss/n_check_step,
